...
select u.id, u.first_name, u.last_name, u.email, a.account_no, a.balance from clients as u left join accounts as a on u.id = a.client_id;
select a.account_no, a.id, t.name from accounts as a left join account_types as t on a.type = t.id;
select t.id, t.amount, tt.name  from transactions as t left join transactions_types as tt on t.transaction_type = tt.id;
select t.id, t.amount, tt.name  from transactions as t right join transactions_types as tt on t.transaction_type = tt.id;

create view transactions_view as select u.id, u.first_name, u.last_name, u.email, a.account_no, a.balance from clients as u left join accounts as a on u.id = a.client_id;
create view acounts_types_view as select a.account_no, a.id, t.name from accounts as a left join account_types as t on a.type = t.id;
create view transactions_types_view as select t.id, t.amount, tt.name  from transactions as t left join transactions_types as tt on t.transaction_type = tt.id;
create view all_transactions_view as select t.id, t.amount, tt.name  from transactions as t right join transactions_types as tt on t.transaction_type = tt.id;

select * from transactions_view;
select * from acounts_types_view;
select * from transactions_types_view;
